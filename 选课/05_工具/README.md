...
配置页会在打开登录浏览器前让你选择监控/自动选课模式、校区、目标课程、教材、登录填充、轮询和分页参数；无参数启动默认选中“模式 A｜自动选课”。模式按钮和“完整预设”按钮都会加载一整套可审计的中文配置，`[*]` 表示当前模式；确认后点击“应用并启动”。

### 中文 TUI 与完整预设

点击“模式 A｜自动选课”会加载自动选课完整预设：只查询 `XGXK` 博雅理论课、请求校区锁定仙林（2）、轮询间隔 1 秒、分页/校区请求间隔 0.5 秒、每页 50 条、单轮最多 50 页、登录等待 300 秒、启用凭据自动填充、教材默认不订、关闭快照。登录后脚本先轮询 `batchisopen.do`；即使课程列表提前可见，服务端未确认轮次开放时也只显示 `PHASE ARMED / open=WAIT`，不会查询候选或提交。开放后才查询课程，并仅在无冲突、未满、未选且当前已选博雅理论课少于 4 门时提交。点击“模式 W｜只监控”会加载同样的查询性能参数，但请求校区为仙林（2）和仙林新北（4），并关闭自动提交。
