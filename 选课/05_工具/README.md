...
NNU 博雅课自动化入口为 nnu_boya_automation.py，默认查询仙林校区和仙林新北校区；使用 `--watch --auto-select --yes --need-book 0/1` 时只持续轮询仙林，直到当前轮次已选非实验课程达到 4 门。可选从 Windows 凭据管理器自动填入学号和密码，但每次启动仍要求本人完成验证码/人机认证。安装、运行和安全闸门见 选课/06_报告/博雅课自动化脚本说明.md。
