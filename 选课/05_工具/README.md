...
配置页会在打开登录浏览器前让你选择监控/自动选课模式、校区、目标课程、教材、登录填充、轮询和分页参数；`MODE-A` 与 `MODE-W` 是互斥的单选行，`[*]` 表示当前模式，点击任意一行即可切换；确认后点击 `APPLY & START`。
...
启动 watch 模式后会先进入可操作配置页：Windows 控制台支持鼠标点击行、滚轮调整数值；`MODE-A`/`MODE-W` 为互斥模式，`[*]` 标记当前选中项，切到 `MODE-W` 会解除自动提交，切到 `MODE-A` 会启用安全闸门；目标行可输入 `auto`、`watch`、`id:<教学班ID>`、`number:<课程号>` 或 `name:<课程名>`，`APPLY & START` 应用设置后才打开登录浏览器；`Q`/`Esc` 取消，键盘 `A` 应用。AUTO-SELECT 的提交校区和安全规则为锁定项，避免误改为不安全请求。
