...
class _MfaPageState extends State<MfaPage> {
  ...
  Future<void> _verify() async {
    ...
    try {
      final result = await widget.api.mfaVerify(code);

      // 只有 success==true 才算登录成功（HTTP 200 不代表登录成功）
      if (result['success'] == true) {
        if (mounted) Navigator.pop(context, true);
        return;
      }

      // 选课时间未到等：显示服务器原始提示，留在登录页
      if (result['access_denied'] == true) {
        final msg = result['error']?.toString() ?? '';
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(msg.isNotEmpty ? msg : '访问被拒绝'),
      ...
      // 其他非成功响应
      _showError(result['error']?.toString() ?? '登录失败');
    } catch (e) {
      final msg = e.toString();
      if (msg.contains('验证码错误')) {
        // 验证码本身错误：state 未消耗，可重试同一 state
        _showError('验证码错误');
        return;
      }
      // 登录失败（如 CAS 拒绝）：MFA state 已消耗，必须重新初始化
      _showError('登录失败，已重新获取验证码，请重试');
      _codeSent = false;
      _codeCtl.clear();
      _countdown = 0;
      _timer?.cancel();
