...
//微专业选课页码
var microPageNumber = 0;
// 微专业选课总页数
var microTotalPage = 0;
// 微专业选课排序方式
var microOrder = null;
...
(function(_card) {
    ...
    function getHtml(_data) {
        var template =
            ...
            '@bookDetail' +
            '<div class="cv-caption-text cv-operation"><button class="cv-btn cv-btn-cancel cv-jxbDetail" type="button" data-hasCourseUrl="@hasCourseUrl" data-courseUrl="@courseUrl" tcId="@detailId" style="margin-left:0px">教学班详情</button>' + 
            ...
            '<button class="cv-btn cv-btn-chose" campus="@campus" teachCampus="@teachCampus" tcId="@tcId" capacitySuffix="@capacitySuffix" hasTest="@hasTest">@btnMsg</button>' +
        ...
        if (hasTest != null && hasTest == '1') {
            btnMsg = '选择' + window.CVParams.sykbs;
        ...
        var bookContent = '';
        if(_data.retakeInfo){
        	bookContent = '<div>' + _data.retakeInfo + '</div>';
        }
...
(function(_list) {
    ...
    function eventsListen($dom) {
        ...
        $dom.children('.cv-list.micro-list').on('click', '.cv-row', function() {
            selectIndex = $(this).attr('index');
            openCourseTeacherList($(this));
        });
        //点击校公选课选择按钮事件
        ...
        $dom.children('.cv-list.micro-list').on('click', '.cv-detail', function(e) {
        	e.stopPropagation();
            var courseNum = $(this).attr('data-num');
            window.open(BaseUrl + '/sys/xsxkapp/*default/coursedetail.do?courseNum=' + courseNum);
        });
    ...
    function init($dom, _data) {
        ...
        if (type === 'recommend') {
        	...
        	microInit($dom, _data.data);
            CVTitleMode.titleInit($('#microSpan'));
            $('#microTitle').html(sysParam.displayNameWZYKC);
        } else if (type == 'school') {
     ...
     * @param _data {Array}
     */
    function microInit($dom, _data) {
        var wrapTemplate = $('#tpl-micro-list').html();
        var rowTemplate = $('#tpl-micro-list-row').html();
        var html = buildCourseListHtml(rowTemplate, _data);
        if (html != null && html != '') {
            wrapTemplate = wrapTemplate.replace('@body', html)
                .replace('@totalPage', microTotalPage)
                .replace("@pageNumber", microPageNumber + 1);
            $dom.html(wrapTemplate);
            // 绑定翻页事件
            $("#microUp").on("click", function(e) {
                pagingCourse('up', 'micro');
            });
            $("#microDown").on("click", function(e) {
                pagingCourse('down', 'micro');
            });
            // 绑定排序事件
            $("#microSort").on("click", function(e) {
                expertModeSorting(e, 'micro');
            });
        } else {
            wrapTemplate = wrapTemplate.replace('@body', noDataHtml)
                .replace('@totalPage', 0)
     ...
     * @param _data {Array} 重载数据列表
     */
    function microReload($dom, _data) {
        var rowTemplate = $('#tpl-micro-list-row').html();
        var html = buildCourseListHtml(rowTemplate, _data);
        if (html != null && html != '') {
            $dom.html(html);
        ...
        // 更新页面的页码
        $('#microTotalPage').html(microTotalPage);
        $('#microPageNumber').html(microPageNumber + 1);
    }
    ...
    function buildCourseListHtml(template, _data) {
        ...
        if (_data != null && _data.length > 0) {
            ...
            for (var i = 0, length = _data.length; i < length; i++) {
                ...
                var replaceCourseNumber = dataItem.replaceCourseNumber?dataItem.replaceCourseNumber:'';
                var replaceCourseName = dataItem.replaceCourseName?dataItem.replaceCourseName:'';
                var replaceCourseDisplay = replaceCourseNumber?'cv-show':'cv-hide';
                var replaceCourseNameDisplay = replaceCourseDisplay;
                ...
                html += template.replace('@number', number)
                    ...
                    .replace('@replaceCourseNumber', replaceCourseNumber)
                    .replace('@replaceCourseNameDisplay', replaceCourseNameDisplay)
                    .replace('@jxb1Number', number)
                    .replace('@hascourseDetail', hascourseDetail)
                    .replace('@replaceCourseDisplay', replaceCourseDisplay)
                    .replace('@replaceCourseName', replaceCourseName)
                    .replace('@title', title)
    ...
    function buildPublicCourseListHtml(template, _data) {
        ...
        if (_data != null && _data.length > 0) {
            ...
            for (var i = 0, length = _data.length; i < length; i++) {
                ...
                var publicCourseTypeName2 = dataItem.publicCourseTypeName2;
                publicCourseTypeName2 = publicCourseTypeName2 == null ? '-' : publicCourseTypeName2;
                ...
                var retakeInfo = dataItem.retakeInfo?dataItem.retakeInfo:'';
 
                html += template.replace('@numberTitle', numberTitle + '[' + index + ']')
                    .replace('@courseNumber', number)
                    .replace('@courseFlagDisplay', courseFlagDisplay)
                    .replace('@courseFlag', courseFlag)
                    .replace('@title', title)
                    ...
                    .replace('@type2', publicCourseTypeName2)
                    .replace('@hours', hours)
                    .replace('@credit', credit)
                    .replace('@volunteerText', volunteerText)
                    .replace('@selectedClass', selectedClass)
                    .replace(/@number1/g, number + '[' + index + ']')
...
(function(_head) {
    ...
    function pageHeadTabChange($a) {
        ...
        } else if (id == '#cvMicroCourse') {
            // 微专业课
            microPageNumber = 0;
            microOrder = '';
            CVMicroCourse.listInit();
        ...
        queryTestCourse(queryParam).done(function(resp){
            ...
            if (code != null && code == '1') {
                ...
                if (dataList != null && dataList.length > 0) {
                    ...
                    tableTplHtml = tableTplHtml.replace('@tbody', tbodyHtml).replace('@theoryClassInfo', theoryClassInfo).replace('@theoryClassTeachingPlace',theoryClassTeachingPlace);
                    ...
                    $('#testCourse_choice_btn').on('click', function(event) {
                        ...
                        if (teachingClassId != null) {
                            choiceTestCourse(jxbid, teachingClassId, $dom, needBook);
                        } else {
                            $.bhTip({
                	...
                	var dialogData = new Object();
                    dialogData.title = '未找到合适的' + window.CVParams.sykbs;
                    dialogData.content = '可能原因： 存在冲突或课容量不够！';
                    CVDialog.showDanger(dialogData);
                }    
            }
        });       
...
(function(_micro) {
     ...
     * 微专业选课列表重载
     */
    _micro.listReload = function(isOpenRow, tcId, capacitySuffix) {
        listReload(isOpenRow, tcId, capacitySuffix);
    };

    function listInit() {
        // 搜索条件内容
        var content = $('#microSearch').val();
        var queryParam = buildQueryTCParam(content);
        queryProgramCourse(queryParam).done(function(resp) {
            var code = resp.code;
            if (code != null && code == '1') {
                // 计算方案选课总页数（向上取整）
            	microTotalPage = Math.ceil(resp.totalCount / pageSize);
                var dataList = resp.dataList;
                var listMock = {
                    type: 'micro',
                    data: dataList
                ...
                courseDataList = dataList;
                CVList.init($('#cvCanSelectMicroCourse'), listMock);
            } else if (code == '302') {
                sessionStorage.removeItem('token');
                sessionStorage.removeItem('studentInfo');
                window.location.href = BaseUrl + '/sys/xsxkapp/*default/index.do';
            } else {
                $('#cvCanSelectMicroCourse').html(resp.msg);
            }
        });
    }
    ...
    function listReload(isOpenRow, tcId, capacitySuffix) {
        ...
        queryProgramCourse(queryParam).done(function(resp) {
            ...
            if (code != null && code == '1') {
                ...
                courseDataList = dataList;
                CVList.reload($('#microBody'), listMock);
                flushTeachingClassCapacity(tcId, capacitySuffix);
                var jxbUnsort = JSON.parse(sessionStorage.getItem('sysParam')).jxbUnsort;
                if (isOpenRow && (jxbUnsort !=1 )) {
                    openRows();
    ...
    function openRows(isOpenRow) {
        var rows = $('.cv-list.micro-list').find('.cv-row');
        for (var i = 0; i < rows.length; i ++) {
            var index = $(rows[i]).attr('index');
            if (selectIndex != null && index == selectIndex) {
                CVList.openRow($(rows[i]));
...
})(window.CVMicroCourse = window.CVMicroCourse || {});
...
function buildQueryTCParam(content) {
    ...
    if (teachingClassType == 'TJKC') {
        ...
        queryData += ',"queryContent":"' + content + '"';
    } else if (teachingClassType == 'WZYKC') {
        // 微专业
        pageIndex = microPageNumber;
        order = microOrder;
        isMajor = '0'; // 是否主修
        var sfct = $('#micro_sfct').val();
        var sfym = $('#micro_sfym').val();
        var kcxz = $('#micro_kcxz').val();
        var kclb = $('#micro_kclb').val();
        var nj = '';
    	var yx = '';
    	var zy = '';
        if(!$('.cv-micro-njyxzy').hasClass('cv-block-hide')){
        	nj = $('#micro_nj').val();
        	yx = $('#micro_yx').val();
        	zy = $('#micro_zy').val();
        }
        ...
        if (nj) {
        	content = 'WZYNJ:' + nj + ',' + content;
        }
        if (yx) {
        	content = 'WZYYX:' + yx + ',' + content;
        }
        if (zy) {
        	content = 'WZYZY:' + zy + ',' + content;
        }
        queryData += ',"queryContent":"' + content + '"';
...
function initMenuControl() {
    ...
    var displayWZYKC = currentBatch.displayWZYKC;
    var displayNameWZYKC = sysParam.displayNameWZYKC;
    ...
    //微专业
    if (displayWZYKC != null && displayWZYKC == '1') {
        var $tab = $("#aMicroCourse");
        $tab.removeAttr("disabled");
        $tab.html(displayNameWZYKC);
        if (initTab == '') {
            initTab = 'micro';
        }
        if(sysParam.displayAllWZY != '1'){
        	$('#aMicroCourse .cv-micro-njyxzy').addClass('cv-block-hide');
        }
    }else if(displayNameWZYKC){
    	$("#aMicroCourse").html(displayNameWZYKC);
    }
    ...
    }else if (initTab == 'micro') {
        // 初始化微专业选课
        teachingClassType = $("#aMicroCourse").attr("teachingClassType");
        sessionStorage.setItem("teachingClassType", teachingClassType);
        CVMicroCourse.listInit();
        $('#cvMicroCourse').attr('class', ' cv-pb-38');
        // 设置选中样式
        $('#aMicroCourse').parent().addClass('cv-active');
...
function pagingCourse(sortType, courseType) {
    if (courseType == 'recommend' && recommendTotalPage > 1) {
        ...
        // 微专业选课翻页
        if (sortType == "up" && microPageNumber > 0) {
        	microPageNumber = microPageNumber - 1;
            CVMicroCourse.listReload();
        } else if (sortType == "down" && microPageNumber < (microTotalPage - 1)) {
        	microPageNumber = microPageNumber + 1;
            CVMicroCourse.listReload();
        }
...
function reloadCourseList(tcId, capacitySuffix) {
    ...
    } else if (tcType == 'WZYKC') {
        CVMicroCourse.listReload(true, tcId, capacitySuffix);
    } else if (tcType == 'QXKC') {
    	CVSchoolCourse.listReload();
    }
}
...
;(function (_search) {
    ...
    function initMicroNj() {
    	queryMicroYxNjZyData('nj').done(function(resp) {
            var code = resp.code;
            if (code != null && code == '1') {
                var dataListTmp = JSON.parse(resp.msg);
                var j = null;
    ...
    function initMicroYx() {
    	queryMicroYxNjZyData('yx').done(function(resp) {
            var code = resp.code;
            if (code != null && code == '1') {
            	var dataListTmp = JSON.parse(resp.msg);
                var j = null;
    ...
    function initZy() {
    	if($('#minor_nj').val() && $('#minor_yx').val()){
    	...
    	}else if($('#micro_nj').val() && $('#micro_yx').val()){
    		queryMicroYxNjZyData('zy').done(function(resp) {
    			var yxValue = $('#micro_yx').val();
    			var njValue = $('#micro_nj').val();
    			var code = resp.code;
    			if (code != null && code == '1') {
    				var dataListTmp = JSON.parse(resp.msg);
    				var j = null;
    		...
    		$("#micro_zy").html('<option value="">请选择</option>');
    	}
    }
