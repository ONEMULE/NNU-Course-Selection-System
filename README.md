# NNU Course Selection System

Research archive and source snapshot for analyzing the NNU course-selection system.

The main materials are organized under [`选课`](选课/):

- `00_研究索引` — collection scope, source lists, and links
- `01_本地证据` — raw local evidence kept off-repository and sanitized evidence for analysis
- `02_公开资料` — official manuals and public reference material
- `03_仓库源码` — public repository source snapshots
- `04_提取分析` — endpoint, field, fingerprint, and comparison outputs
- `05_工具` — repeatable PowerShell collection and analysis helpers
- `06_报告` — report workspace

Session-bearing browser captures and their resource folders are intentionally kept local and excluded by the top-level `.gitignore`. Sanitized copies are included for reproducible analysis.
